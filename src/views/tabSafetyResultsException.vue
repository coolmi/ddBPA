    ...
    <div class="all">
        ...
        <div class="title_left">
          <table class="main_table">
            <tr v-for="(val,index) in list2" :key="index">
              ...
              <th style="width: 80px;font-size: 14px">{{val.b}}</th>
        ...
        <div class="content_left">
          <table class="main_table">
            <tr v-for="(item,index) in list" :key="index">
              ...
              <td style="width: 110px;color: deepskyblue;font-size: 14px" @click="showCall(item.mobile)">{{item.mobile}}</td>
              <td style="width: 80px;font-size: 14px">{{item.lastplace}}</td>
      ...
      <div>
        <div style="display: flex;justify-content: space-between; align-items: center">
          <!--<div style="width: 50%;">-->
            <!--<x-input title="所属组织" v-model="orgname" style="color: #000;font-size: 14px" placeholder="请输入所属组织"></x-input>-->
          <!--</div>-->
          <div style="width: 50%;">
        ...
        <x-button text="查询" @click.native="getSafetyResultsException(leadername, dateStr)" :gradients="['#1D62F0', '#19D5FD']"></x-button>
      </div>
    </div>
  </div>
</template>
...
<script>
  import api from 'api'
  import { XHeader, XInput, Datetime, XButton, XSwitch } from 'vux'
  ...
  export default {
    ...
    created() {
      ...
      // });
      _that.getSafetyResultsException('', '');
    ...
    methods: {
      ...
      getSafetyResultsException (leadername, dateStr) {
        let _that = this;
        // alert(orgname + dateStr)
        api.getSafetyResultsException(leadername, dateStr, function (res) {
      ...
      getSafetyResults (ename, mobile, flag) {
        ...
        api.getSafetyResults(ename, mobile, rep, function (res) {
          // alert(JSON.stringify(res.data.data.safetyresult))
          _that.list = res.data.data.safetyresult;
        })
      }
